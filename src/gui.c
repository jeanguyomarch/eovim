...
static void _wildmenu_resize(struct gui *gui);
static void _tabs_shown_cb(void *data, Evas_Object *obj, const char *emission, const char *source);

static inline struct nvim *_nvim_get(const Evas_Object *obj)
{
...
static Evas_Object *_layout_item_add(Evas_Object *parent, const char *group)
{
...
static inline Evas_Object *_compl_text_set(Evas_Object *layout, const struct gui *gui,
					   const char *text)
{
	Evas_Object *const edje = elm_layout_edje_get(layout);
	edje_object_text_class_set(edje, "completion_text", gui->font.name, (int)gui->font.size);
	elm_layout_text_set(layout, "text", text);
	return edje;
}

static void _spacer_set(Evas_Object *obj, size_t char_w, size_t char_h, size_t len)
{
...
static Evas_Object *_compl_content_get(void *data, Evas_Object *obj, const char *part)
{
	const struct completion *const cmpl = data;
	struct nvim *const nvim = _nvim_get(obj);
	struct gui *const gui = (struct gui *)(&nvim->gui);
	Evas_Object *o;

	if (!eina_streq(part, "elm.swallow.content")) {
		return NULL;
	}
    ...
    * string, then we don't bother with it. Nothing will be shown. */
	if (cmpl->kind[0] != '\0') {
		...
		Evas_Object *const kind_edje = _compl_text_set(kind, gui, cmpl->kind);
		...
		const Edje_Message_String msg = { .str = (char *)cmpl->kind };
		edje_object_message_send(kind_edje, EDJE_MESSAGE_STRING, THEME_MSG_COMPLETION_KIND,
					 (void *)(&msg));
	}
	...
	_compl_text_set(o, gui, cmpl->menu);
	elm_table_pack(table, o, 1, 0, 1, 1);
	evas_object_show(o);
	...
	_compl_text_set(o, gui, cmpl->word);
	elm_table_pack(table, o, 2, 0, 1, 1);
	evas_object_show(o);
...
static void _completion_sel_cb(void *data, Evas_Object *obj EINA_UNUSED, void *event)
{
	struct gui *const gui = data;
    ...
    * clicked on and we want to insert. */
	const int sel_idx = gui->completion.sel ? elm_genlist_item_index_get(gui->completion.sel) :
...
void gui_completion_add(struct gui *gui, struct completion *cmpl)
{
	elm_genlist_item_append(gui->completion.gl, _compl_itc, cmpl, NULL, ELM_GENLIST_ITEM_NONE,
				_completion_sel_cb, gui);
}

static Elm_Object_Item *_gl_nth_get(const Evas_Object *gl, unsigned int index)
{
	/*
			 ...
			 unsigned int x, unsigned int y)
{
    ...
    */
	...
	if (selected >= 0) {
		...
		Elm_Object_Item *const sel =
			_gl_nth_get(gui->completion.gl, (unsigned int)selected);
    ...
    */
	...
	/* Cell' size */
	int char_w, char_h;
	termview_cell_size_get(gui->termview, (unsigned int *)(&char_w), (unsigned int *)(&char_h));
    ...
    */
	const int ideal_width =
		(int)(gui->completion.max_type_len + gui->completion.max_word_len + 6) * char_w;

	/* This is the ideal height. We add 7 because it looks nice.
    * Maybe this will change with various fonts... that's lame. */
	const int ideal_height = (char_h + 7) * (int)gui->completion.items_count;
...
static void _compl_item_del(void *data, Evas_Object *obj EINA_UNUSED)
{
	struct completion *const cmpl = data;
	nvim_completion_free(cmpl);
}

static void _wildmenu_item_del(void *data, Evas_Object *obj EINA_UNUSED)
{
	Eina_Stringshare *const item = data;
	eina_stringshare_del(item);
}

static Evas_Object *_wildmenu_content_get(void *data, Evas_Object *obj,
...
static void _wildmenu_sel_cb(void *data, Evas_Object *obj EINA_UNUSED, void *event)
{
	struct gui *const gui = data;
	const Elm_Object_Item *const item = event;
...
void gui_wildmenu_append(struct gui *gui, Eina_Stringshare *item)
{
	Elm_Object_Item *const wild_item =
		elm_genlist_item_append(gui->cmdline.menu, _wildmenu_itc, item, NULL,
					ELM_GENLIST_ITEM_NONE, _wildmenu_sel_cb, gui);
...
static void _wild_item_realized(void *data, Evas_Object *obj, void *info EINA_UNUSED)
{
