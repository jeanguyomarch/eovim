...
static struct termview_style *
_termview_style_new(void)
{ return calloc(1, sizeof(struct termview_style)); }

static void
_termview_style_free(struct termview_style *const style)
{ free(style); }

static Eina_Bool
_style_foreach(
  const Eina_Hash *const hash EINA_UNUSED,
  ...
  void *const fdata)
{
  ...
  if (style->reverse)
  {
    const uint32_t fg = (style->bg_color.value == COLOR_DEFAULT)
      ? sd->style.default_bg.value : style->bg_color.value;
    const uint32_t bg = (style->fg_color.value == COLOR_DEFAULT)
      ? sd->style.default_fg.value : style->fg_color.value;
    eina_strbuf_append_printf(buf,
        " color=#%06"PRIx32" backing=on backing_color=#%06"PRIx32,
...
static void
_termview_style_update(struct termview *const sd)
{
  Eina_Strbuf *const buf = sd->style.text;
  ...
  eina_strbuf_append_printf(buf,
    "DEFAULT='font=\\'%s\\' font_size=%u color=#%06x wrap=none",
  ...
  evas_textblock_cursor_line_geometry_get(
    sd->cursors[0], NULL, NULL, NULL, (int*)&sd->cell_h);
...
static void
_cursor_calc_vertical(
  struct termview *const sd,
  const int x, const int y, int w, const int h)
{
  ...
  const int x, const int y, const int w, const int h)
{
  ...
  if (h2 <= 0) h2 = 1; /* We don't want an invisible cursor */
                      ...
                      void *event EINA_UNUSED)
{
   ...
   edje_object_message_send(sd->cursor.ui, EDJE_MESSAGE_INT,
...
static void
_relayout(struct termview *const sd)
{
...
termview_init(void)
{
   ...
   static const Evas_Smart_Cb_Description smart_callbacks[] =
   {
     {"relayout", ""},
     {NULL, NULL}
   };
...
void
termview_clear(Evas_Object *const obj)
{
  struct termview *const sd = evas_object_smart_data_get(obj);
...
void
termview_flush(Evas_Object *const obj)
{
  struct termview *const sd = evas_object_smart_data_get(obj);
  ...
  for (unsigned int i = 0u; i < sd->rows; i++)
  {
    if (! sd->line_has_changed[i]) { continue; }
    ...
    for (unsigned int col = 0u; col < sd->cols; col++)
    {
      ...
      if (c->style_id != last_style)
      {
        ...
        if (c->style_id != 0)
        { eina_strbuf_append_printf(line, "<X%"PRIx32">", c->style_id); }
      }
    ...
    evas_object_textblock_text_markup_prepend(end,
      eina_strbuf_string_get(line));
...
void
termview_cursor_goto(Evas_Object *const obj,
  ...
  const int rows)
{
  ...
  for (int from_line = start_line; from_line != end_line; from_line += step)
  {
    ...
    if ((unsigned int)to_line >= sd->rows) { continue; }
                        ...
                        int *py)
{
   ...
   if (px) *px = (int)(cell_x * sd->cell_w);
   if (py) *py = (int)(cell_y * sd->cell_h);
}

void
termview_cursor_mode_set(Evas_Object *const obj,
                         const struct mode *const mode)
{
  ...
  if (style != NULL && style->fg_color.value != COLOR_DEFAULT)
  {
    ...
    edje_object_message_send(sd->cursor.ui, EDJE_MESSAGE_INT_SET,
...
{
   struct termview *const sd = evas_object_smart_data_get(obj);
   if (visible) evas_object_show(sd->cursor.ui);
   else evas_object_hide(sd->cursor.ui);
}
...
{
  struct termview *const sd = evas_object_smart_data_get(obj);
  const Eina_Bool changed =
    (sd->style.default_fg.value != fg.value) ||
...
void
termview_font_set(Evas_Object *const obj,
  Eina_Stringshare *const font_name,
  const unsigned int font_size)
{
  ...
  evas_object_textgrid_font_set(sd->sizing_textgrid,
    sd->style.font_name, (int)sd->style.font_size);
  evas_object_textgrid_cell_size_get(sd->sizing_textgrid,
    (int*)&sd->cell_w, (int*)&sd->cell_h);
  sd->pending_style_update = EINA_TRUE;
}

void
termview_linespace_set(
  Evas_Object *const obj,
  const unsigned int linespace)
{
  struct termview *const sd = evas_object_smart_data_get(obj);
