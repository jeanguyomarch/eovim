...
#define GEN_DECODERS(Kw, DecodeFunc, FieldName) \
  static Eina_Bool \
  _attr_ ## Kw ## _cb(const msgpack_object *const obj, struct termview_style *const style) \
  ...
  const msgpack_object_array *const args)
{
  ...
  const msgpack_object_array *const params =
    array_of_args_extract(args, args->size - 1);
  ...
  const msgpack_object_array *const args)
{
  ...
  for (uint32_t i = 1u; i < args->size; i++)
  {
    ...
    struct termview_style *const style =
      termview_style_get(nvim->gui.termview, id);
  ...
  const msgpack_object_array *const args)
{
  ...
  for (uint32_t i = 1u; i < args->size; i++)
  {
    ...
    for (uint32_t j = 0u; j < cells->size; j++)
    {
      ...
      termview_line_edit(nvim->gui.termview,
        (unsigned int)row, (unsigned int)col,
        str->ptr, (size_t)str->size,
        (uint32_t)style_id, (size_t)repeat);
  ...
  const msgpack_object_array *const args)
{
  ...
  for (uint32_t i = 1u; i < args->size; i++)
  {
    ...
    termview_scroll(nvim->gui.termview,
