...
struct arg {
	...
	const enum opt short_opt; /**< Option identifier */
};

#define ARG(Long, Short)                                                                           \
	{                                                                                          \
		.long_opt = Long, .short_opt = Short                                               \
	}

static const struct arg _args[] = {
	ARG("nvim", OPT_NVIM),
...
static int _find_long_option(const char *arg, const char **ret)
{
...
static int _find_short_option(const char *arg)
{
	/* Match the character after the leading '-'. E.g. "-o" shall match 'o' */
	for (size_t i = 0; i < _args_size; i++)
		if ((enum opt)(arg[1]) == _args[i].short_opt)
			return _args[i].short_opt;
...
static Eina_Bool _parse_geometry(struct geometry *geo, const char *str)
{
...
{
	enum options_result res = OPTIONS_RESULT_CONTINUE;
...
#define GET_NEXT_ARG() param ? param : argv[++i]
			...
			switch (opt) {
				/* We make eovim catch some neovim options to make sure they
                   * are not used! */
			case OPT_FORBIDDEN:
				fprintf(stderr, "eovim: Forbidden option argument: \"%s\"\n", it);
